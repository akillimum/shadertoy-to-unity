Shader "ShaderToy/URP/Protean clouds 3l23Rh"
{
    Properties
    {
...
float2 map(float3 p)
 {
    float3 p2 = p;
    p2.xy -= disp(p.z).xy;
    p.xy = mul(rot(mul(sin(p.z + _Time.y) , (0.1 + prm1 * 0.05)) + _Time.y * 0.09), p.xy);
    float cl = mag2(p2.xy);
    float d = 0.;
    p *= .61;
    float z = 1.;
...
UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
 ...
 float2 fragCoord = ((input.screenPos.xy) / (input.screenPos.w + FLT_MIN)) * _ScreenParams.xy;
     ...
     float2 p = (input.positionCS.xy - 0.5 * _ScreenParams.xy) / _ScreenParams.y;
