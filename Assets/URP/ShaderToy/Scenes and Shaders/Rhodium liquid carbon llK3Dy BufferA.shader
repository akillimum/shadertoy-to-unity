Shader "ShaderToy/URP/Rhodium liquid carbon llK3Dy BufferA"
{
    Properties
    {
...
UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
 ...
 float2 fragCoord = ((input.screenPos.xy) / (input.screenPos.w + FLT_MIN)) * _ScreenParams.xy;
     ...
     // bouncy cam every 10 seconds 
        ...
        // camera 
           float3 dir = normalize(float3 (2. * input.positionCS.xy - res.xy , res.y));
              ...
              float t = castRayx(org , dir);
               ...
               // lighting: 
                  ...
                  // scene depth 
                     ...
                     if (t < 12.0)
                      {
                         float dsf = max(dot(lig, nor), 0.) + pow(max(dot(reflect(dir, nor), lig), 0.), 16.);
                          color2 = float3 (dsf, dsf, dsf);
                     ...
                     // scene depth included in alpha channel 
                        fragColor = float4 (float3 (1. * color + 0.8 * color2) * 1.3 , abs(0.67 - depth) * 2. + 4. * wobble);
                     return fragColor;
                    }
